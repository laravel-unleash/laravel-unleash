...
class UnleashFake
{
    ...
    protected function getFakeFeature($feature, ?array $args = [])
    {
        ...
        if ($featureFound ->testArgs !== null) {
            $flag->withTestArgs(... $featureFound->testArgs);
        }
        return $flag;
    }
    ...
    protected function _enabled($name, array $args): bool
    {
        ...
        $status = $flag->enabled(... $args);
        if (!is_callable($flag->testArgsUsing) && $flag->testArgs !== null && $flag->testArgs !== $args) {
